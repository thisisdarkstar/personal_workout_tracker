...
export default function DayNavigator({
  currentWeek, currentDay, onWeekChange, onDayChange,
  weekInfo, startDate, completedWorkouts = {},
}) {
  const phase = getPhase(currentWeek)
  const phaseData = PHASES[phase]
  const [showCalendar, setShowCalendar] = useState(false)
  const scrollRef = useRef(null)
  const isViewingToday = weekInfo && currentWeek === weekInfo.todayWeek && currentDay === weekInfo.todayDay

  useEffect(() => {
    if (weekInfo?.planStarted && scrollRef.current) {
      const button = scrollRef.current.children[weekInfo.todayDay]
      if (button) {
        button.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' })
      }
    }
  }, [currentWeek, weekInfo?.planStarted, weekInfo?.todayDay])
  ...
  return (
    <div className="bg-boxing-panel rounded-2xl p-4 mb-4 pb-2">
      ...
      <div className="flex items-center gap-2 mb-3">
        <button
          onClick={() => { if (currentWeek > 1) { onWeekChange(currentWeek - 1); onDayChange(0) } }}
        ...
        <div ref={scrollRef} className="flex-1 flex gap-1.5 overflow-x-auto scrollbar-thin px-1">
          {DAYS.map((day, index) => {
            const isSelectedDay = currentDay === index
            const isTodayDay = weekInfo && currentWeek === weekInfo.todayWeek && index === weekInfo.todayDay
            const hasDone = Object.keys(completedWorkouts).some(k => k.startsWith(`${currentWeek}-${index}-`))

            return (
              <button
                key={index}
                onClick={() => onDayChange(index)}
                className={`relative flex-1 min-w-[44px] h-14 rounded-xl text-sm font-semibold transition-all active:scale-95 flex flex-col items-center justify-center ${
                  isSelectedDay
              ...
              >
                <div className="truncate px-1 leading-none">{day.slice(0, 2)}</div>
                <div className={`text-[10px] mt-0.5 ${isSelectedDay ? 'text-boxing-dark/70' : 'text-gray-500'}`}>
                  {index + 1}
                </div>
                {/* Completion dot */}
                {hasDone && (
                  <div
                    className={`absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${
                      isSelectedDay ? 'bg-boxing-dark/50' : 'bg-boxing-neon'
                    }`}
                  />
                )}
              </button>
        ...
        <button
          onClick={() => { if (currentWeek < 12) { onWeekChange(currentWeek + 1); onDayChange(0) } }}
      ...
      <div className="text-center pb-1">
        <div className="flex items-center justify-center gap-2">
          <span className="text-gray-500 text-sm font-medium">
            {DAYS[currentDay]} · Week {currentWeek}
          </span>
          {DAY_TYPES[currentDay].type === 'fullRest' && (
            <span className="px-2 py-0.5 bg-purple-500/20 text-purple-400 text-xs rounded-full">Rest</span>
          )}
          {DAY_TYPES[currentDay].type === 'activeRest' && (
            <span className="px-2 py-0.5 bg-blue-500/20 text-blue-400 text-xs rounded-full">Active Rest</span>
          )}
        </div>
      </div>
    </div>
...
function CalendarView({ currentWeek, currentDay, onWeekChange, onDayChange, onClose, weekInfo, startDate }) {
  ...
  const getWeekStart = (weekNum) => {
    ...
    const daysToSubtract = (planStart.getDay() + 6) % 7
  ...
  return (
    <div className="mb-4 p-3 bg-boxing-dark rounded-xl">
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                const d = Math.floor((day.getTime() - today.getTime()) / 86400000)
                onWeekChange(Math.max(1, Math.min(12, todayWeek + Math.floor(d / 7))))
                onClose()
              }}
              className={`flex flex-col items-center justify-center p-2 rounded-lg transition ${
                isToday ? 'bg-boxing-neon text-boxing-dark font-bold' : 'bg-boxing-ring text-gray-400 hover:bg-boxing-panel'
              }`}
            >
              <div className="text-[10px] opacity-70">{['Mo','Tu','We','Th','Fr','Sa','Su'][dayIndex]}</div>
      ...
      <button
        onClick={() => { onWeekChange(todayWeek); onDayChange(todayDay); onClose() }}
        className="w-full py-2 text-sm bg-boxing-neon text-boxing-dark font-medium rounded-lg hover:bg-boxing-neon/80 transition"
      >
        Jump to Today
      </button>
    </div>
